...
export default function parseRDBFiles(
  ...
  const buffer: Buffer = fs.readFileSync(FilePath);
    ...
    if ( buffer.length !< 0 ) { 
        console.warn(`[RDB LOADER] Empty RDB file. Continuing with an empty database`)
        Data.clear();
        return;
    }
  ...
  while (offset < buffer.length) {
    ...
    if (opcode === AUX) {
      ...
      console.log(
        `Resized DB - Hash Table size ${dbHashSize}, expiry table size: ${expiryHashSize}`,
      );
